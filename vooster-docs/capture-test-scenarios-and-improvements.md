...
- **API 광고주 연동**: 촬영/확인 페이지에서 `useAdvertisers()` 사용. 로딩 시 "광고주 목록 불러오는 중…", 실패 시 "광고주 목록을 불러오지 못했습니다. 매칭이 제한될 수 있습니다." 안내 표시. **(구현 완료)**
- **매칭 실패 시 촬영본 보관**: "일단 저장" 버튼으로 광고주 미매칭 이미지도 세션에 추가 가능. **(구현 완료)** “” 
### 2.2 UX

- **촬영 직후 피드백**: 매칭 성공 시 “OO 광고주로 인식됨” 등 선택적 안내 가능.
- **위치 없음 취소**: "위치 없음" 선택 후 “위치 촬영으로 되돌리기” 버튼이 없음. 필요 시 상태 초기화 버튼 추가 검토.

### 2.3 확인 페이지

- **위치 없음 세션 라벨**: "위치 없음 세션" 라벨로 역명·호선 미촬영 구분 표시. **(구현 완료)**
- **보고 발송**: Resend 연동으로 실제 발송·이력 저장( vision_ocr_reports ) 구현. **(구현 완료)**

### 2.4 성능 · 안정성

- **OCR 부하**: 서버 OCR 분당 rate limit(30회/분), 429 시 클라이언트 토스트 "OCR 제한". 광고 촬영 시 버튼 "OCR 인식 중..." 로딩 표시. **(구현 완료)**
- **세션 크기**: 촬영 이미지 저장 전 `compressDataUrl`(최대 1200px·품질 0.75) 적용. 8장 이상 시 "저장 용량 참고" 안내, QuotaExceededError 시 "저장 공간 초과" 토스트. **(구현 완료)**
